...
- (void)viewDidLoad {
    ...
    _parametersConfigStep2AP=[[ParametersConfig alloc]init:self ip:@"192.168.100.1" password:@"admin"];
}
...
- (void)_AddDeviceStep2APNextClick{
    ...
    [_parametersConfigStep2AP joinWifi:_AddDeviceStep2APNetField.text :_AddDeviceStep2APPskField.text];
}
    ...
    if (type==JOIN_WIFI) {
        ...
        if(statusCode==200){
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSRange range=[body rangeOfString:key];
                if (range.location != NSNotFound) {
                    int i=(int)range.location;
                    NSString *jsonString=[body substringFromIndex:i+key.length];
                    NSLog(@"%@",jsonString);
...
-(void)GetWifiList{
    getTime=20;
    [_parametersConfigStep2AP getSsidList];
    while (getTime>0) {
        if (getTime==1) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [waitGetAlertView dismissWithClickedButtonIndex:0 animated:YES];
