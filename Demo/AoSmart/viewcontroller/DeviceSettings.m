...
- (void)viewDidLoad {
    ...
    _parametersConfigDeviceSettings=[[ParametersConfig alloc]init:self ip:deviceConnectingIp password:deviceConnectingpsk];
    [_parametersConfigDeviceSettings getFps:0];
}

- (void)setOnResultListener:(int)statusCode :(NSString*)body :(int)type{
    if (type==GET_FPS) {
        if(statusCode==200){
            _DeviceSettingsFPSField.text=[self parseJsonString:[body stringByReplacingOccurrencesOfString:@" " withString:@""]];
            [_parametersConfigDeviceSettings getQuality:0];
        }
    }
    else if(type==GET_QUALITY) {
        if(statusCode==200){
            _DeviceSettingsQualityField.text=[self parseJsonString:[body stringByReplacingOccurrencesOfString:@" " withString:@""]];
            [_parametersConfigDeviceSettings getGOP];
        }
    }
    else if(type==GET_GOP) {
        if(statusCode==200){
            _DeviceSettingsGOPField.text=[self parseJsonString:[body stringByReplacingOccurrencesOfString:@" " withString:@""]];
        }
    }
    else if (type==UPDATE_USERNAME_PASSWORD){
        if(statusCode==200)
        {
        ...
        if(statusCode==200)
        {
            [_parametersConfigDeviceSettings setQuality:0 :_DeviceSettingsQualityField.text];
        }
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(statusCode==200){
            [_parametersConfigDeviceSettings setGOP:_DeviceSettingsGOPField.text];
        }
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)modifyDevicePassword{
    [_parametersConfigDeviceSettings updateUsernameAndPassword:@"admin" :_DeviceSettingsNewPskField.text];
}
...
-(void)modifyDeviceParameters{
    ...
    [_parametersConfigDeviceSettings setFps:0 :_DeviceSettingsFPSField.text];
}
