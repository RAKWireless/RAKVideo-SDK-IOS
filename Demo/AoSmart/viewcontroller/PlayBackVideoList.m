...
- (void)viewDidLoad {
    ...
    _parametersConfigVideoList=[[ParametersConfig alloc]init:self ip:deviceConnectingIp password:deviceConnectingpsk];
    [_parametersConfigVideoList getVideoList:folderName];
...
- (void)setOnResultListener:(int)statusCode :(NSString*)body :(int)type{
    if (type==GET_VIDEO_LIST) {
        if(statusCode==200){
            ...
            NSLog(@"folderlist=%@",body);
            [self parseJsonString:body];
